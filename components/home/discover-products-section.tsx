...
import { getDiscoverProducts, DiscoverProduct, FALLBACK_DISCOVER_PRODUCTS } from "@/services/discover-products-service"
...
export function DiscoverProductsSection() {
  ...
  const renderTitle = (product: DiscoverProduct) => {
    if (product.title && product.title.startsWith("discoverProducts.")) {
      return t(product.title as any)
    }
    return product.title
  }

  const renderDescription = (product: DiscoverProduct) => {
    if (product.description && product.description.startsWith("discoverProducts.")) {
      return t(product.description as any)
    }
        ...
        {loading ? (
          ...
          >
            {visibleProducts.map((product) => {
              const titleText = renderTitle(product)
              const descText = renderDescription(product)
                ...
                >
                  <article className="flex h-full flex-col overflow-hidden rounded-[28px] border border-[#324198]/8 bg-white p-4 shadow-[0_12px_35px_rgba(15,23,42,0.06)] transition-all duration-500 group-hover:-translate-y-1.5 group-hover:border-[#324198]/15 group-hover:shadow-[0_18px_45px_rgba(15,23,42,0.1)] group-focus-visible:-translate-y-1.5 group-focus-visible:border-[#324198]/15">
                    ...
                    <div className="flex flex-1 flex-col pt-5">
                      ...
                      <p className="mt-3 font-cairo text-sm leading-7 text-[#667089] line-clamp-3">
                        {descText}
                      </p>
